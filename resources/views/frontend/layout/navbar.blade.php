    ...
    <div class="container-fluid">
        <div class="row flex-wrap v-center">
            ...
            <div class="col-xs-12 col-md-4 text-center text-md-right ">
                ...
                <div class="dropdown search-dropdown"> <a id="search" data-target="#" href="./"
                            ...
                            <form method="get" class="searchform form-inline" action="./">
                                <div class="form-group-wrap">
                                    <div class="form-group with_button margin_0"> <label class="sr-only"
                                            for="topline-search">Search for:</label> <input id="topline-search"
                                            type="text" value="" name="search" class="form-control"
                                            placeholder="Search Keyword"> <button type="submit"
                                            class="theme_button">Search</button> </div>
                                </div>
                            </form>
                        </div> <i class="qtyler-close dropdown-close" aria-hidden="true"></i>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="col-sm-12 display-flex v-center">
                    <div class="header_left_logo">
                        <a href="{{ route('home') }}" class="logo">
                            <img src="{{ asset($wss->logo ?? '') }}" alt="logo">
                        </a>
                    </div>
                                ...
                                <li> <a href="#">Products</a>
                                    <ul>
                                        ...
                                        @foreach (App\product_division::all() as $row)
                                            <li>
                                                <a
                                                    href="{{ route('division', $row->id) }}">{{ $row->division_name }}</a>
                                                @php
                                                ...
                                                @if (count($category) > 0)
                                                    <ul>
                                                        @foreach ($category as $cat)
                                                            <li>
                                                                <a
                                                                    href="{{ route('category', ['div_id' => $row->id, 'cat_id' => $cat->id]) }}">{{ $cat->category_name }}</a>
                                                                @php
                                                                ...
                                                                @if (count($gender) > 0)
                                                                    <ul>
                                                                        @foreach ($gender as $gen)
                                                                            <li>
                                                                                <a
                                                                                    href="{{ route('products_view', ['div_id' => $row->id, 'cat_id' => $cat->id, 'gen_id' => $gen->id]) }}">{{ $gen->gender_name }}</a>
                                                                            </li>
                                                                        @endforeach
                                                                    </ul>
                                                                @endif
