            ...
            <div class="row">
                ...
                <aside class="col-xs-12 col-sm-6 col-md-3 col-lg-3 col-md-pull-9 col-lg-pull-9 col-sm-offset-3 col-md-offset-0">
                    <div class="widget widget_categories">
                        ...
                        <ul class="greylinks">
                            @foreach($division->categories as $row)
                                <li> 
                                    <a @if($row->id == $category->id) class="active" @endif href="{{ route('category', ['div_id' => $division->id, 'cat_id' => $row->id]) }}">
                                        {{ $row->category_name }}
                                    </a> <span>{{$row->products->count()}}</span> 
                                </li>
                            @endforeach
                        </ul>
                    </div>
