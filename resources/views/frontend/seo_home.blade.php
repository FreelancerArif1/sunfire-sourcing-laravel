...
@section('meta_description', 'Incorporated in 2026, Sunfire Sourcing is a well-established globally reputed garments apparel buying, sourcing, and management services company in Dhaka, Bangladesh.')
@section('meta_keywords', 'garments, sourcing, bangladesh')
			...
			<div class="row">
				<div class="col-xs-12 text-center">
					...
					<div class="filters carousel_filters"> 
						...
						<a href="#" data-filter=".{{$row->id}}">{{$row->division_name}}</a> 
						@endforeach
					</div>
					<div class="owl-carousel products-carousel gallery-carousel" data-nav="true" data-dots="false"
					    ...
					    @foreach($division as $row)
					    	@foreach(App\Product::where('division',$row->id)->orderByDesc('id')->limit(20)->get() as $product)
								<article class="product vertical-item text-center with-corner-label {{$row->id}}">
