...
class HomeController extends Controller
{
    ...
    public function home2(Request $request)
    {
        ...
        curl_setopt_array($curl, array(
          CURLOPT_URL => 'https://ipinfo.io/'.$request->ip().'/json',
        ...
        if(isset($response->ip)){
            $check = Visitor::where('ip', $response->ip)->first();
            if ($check) {
                $check->number_of_time = $check->number_of_time + 1;
                $check->save();
            }else{
                $visitor = new Visitor();
                $visitor->number_of_time  = 1;
                $visitor->ip   = $response->ip ?? '';
                $visitor->hostname  = $response->hostname ?? '';
                $visitor->anycast  = $response->anycast ?? '';
                $visitor->city     = $response->city ?? '';
                $visitor->region   = $response->region ?? '';
                $visitor->country  = $response->country ?? '';
                $visitor->loc  = $response->loc ?? '';
                $visitor->org  = $response->org ?? '';
                $visitor->postal   = $response->postal ?? '';
                $visitor->timezone   = $response->timezone ?? '';
                $visitor->readme = $response->readme ?? '';
                $visitor->save();
            }
        }
    ...
    public function our_clients()
    {
        ...
        $data = Client::all();
        return view('frontend.our-clients')->with('title', 'Our Clients')
            ->with('division',$division)
            ->with('category',$category)
            ->with('gender',$gender)
            ->with('data', $data)->with('service', $service)
    ...
    public function division($div_id)
    {
        ...
        return view('frontend.division', compact('products', 'division'));
    ...
    public function category($div_id, $cat_id)
    {
        ...
        return view('frontend.product_category_view', compact('products','category', 'division'));
    ...
    public function products_view($div_id, $cat_id, $gen_id)
    {
        $products = DB::table('products')
            ...
            ->distinct()
            ->get();
        $division = product_division::find($div_id);
        $category = product_category::find($cat_id);
        $gender = product_gender::find($gen_id);
        // var_dump($products);
        // exit();

        return view('frontend.products_view', compact('products','division','category','gender'));
    }
    ...
    public function career_store(Request $request)
    {
        ...
        return redirect()->back()->withErrors(['msg' => 'Application submited successfully!']);
    ...
    public function career_mail(Career $career)
    {
        ...
        $emailTo = 'developer.abusaid@gmail.com';
